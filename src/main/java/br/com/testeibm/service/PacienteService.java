...
public class PacienteService {
	...
	public void getPaciente() {
		...
		double avg = list.stream().filter(p -> p.getSexo().equals(sexo)).map(p -> p.getIdade()).reduce((int) 0.0,
				(x, y) -> x + y) / list.size();
		System.out.println("Idade m�dia dos homens = " + avg);
		
////
		List<String> qtdMulher = list.stream()
				.filter(p -> p.getAltura() > 1.60)
				.filter(p -> p.getAltura() < 1.70)
				.filter(p -> p.getPeso() > 70)
				.filter(p -> p.getSexo().equals("Fem"))
				.map(p -> p.getNome()).sorted(comp)
				.collect(Collectors.toList());
